...
export function RecentActivitySection({ activities = [], isLoading = false }: RecentActivitySectionProps) {
    return (
        <div className="bg-zinc-900 rounded-2xl p-4 md:p-6 shadow-lg">
            <SectionHeader
                icon={<LightningBoltIcon className="w-4 h-4 sm:w-7 sm:h-7 text-yellow-300 drop-shadow-[0_0_6px_rgba(253,224,71,0.7)]" />}
                title="Recent Activity"
            />
            {isLoading ? (
                <div className="text-cyan-400 text-center py-6 animate-pulse text-lg">Loading activity...</div>
            ) : activities.length === 0 ? (
                <div className="text-zinc-400 text-center py-6 text-lg italic">Coming soon: Your recent activity will appear here!</div>
            ) : (
