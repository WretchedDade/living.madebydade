...
export function CustomToast({ title, description, action, variant = 'success', t }: CustomToastProps & { t?: any }) {
    ...
    return (
        <div className={`bg-zinc-900 border ${styles.borderColor} ${styles.shadowColor} shadow-md rounded-lg text-cyan-200 font-mono px-4 py-2 flex flex-col gap-2 sci-fi-glow relative`}>
            <button
                type="button"
                aria-label="Dismiss"
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-1">
                    <div className={`${styles.titleColor} font-bold tracking-wide text-base sci-fi-title-glow`}>{title}</div>
                    {description && <div className={`${styles.descColor} font-mono text-xs leading-tight`}>{description}</div>}
...
export function showToast({ title, description, action, variant = 'success' }: ShowToastProps) {
    toast.custom(t => (
        <CustomToast t={t} title={title} description={description} action={action} variant={variant} />
    ));
}
