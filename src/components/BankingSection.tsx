...
function AccountCard({ account }: { account: Account }) {
	...
	return (
		<div key={account.account_id} className="bg-card rounded-xl shadow p-4">
			{account.institution != null ? (
				<div className="flex items-center gap-3 mb-2">
					<span className="font-bold text-primary text-lg">{account.institution.name}</span>
				</div>
			) : null}
			<div className="flex items-center justify-between mb-1">
				<span className="font-semibold text-primary">{account.name}</span>
				<span className="font-mono text-primary text-base">{formatBalance(primaryAmount, currency)}</span>
			</div>
...
export function BankingSection() {
	...
	return (
		<div className="flex flex-col flex-1 bg-background rounded-2xl p-6 shadow-lg">
			<SectionHeader
				icon={
			...
			{accountsQuery.isSuccess && accountsQuery.data.length === 0 && (
				<div className="items-center justify-center flex flex-col grow w-full">
					<div className="flex flex-col items-center justify-center my-auto py-8">
						<IdentificationIcon className="w-8 h-8 text-primary mb-2" />
						<p className="text-muted-foreground text-center text-lg italic mb-2">No bank accounts linked yet</p>
