	...
	async function handleSubmit(value: BillFormValues) {
		...
		if ((bill.dayDue ?? undefined) !== (value.dueType === "Fixed" ? Number(value.dayDue) : undefined))
			changes.push({
				field: "dayDue",
				before: bill.dayDue,
				after: value.dueType === "Fixed" ? Number(value.dayDue) : undefined,
			});
		if (bill.isAutoPay !== value.isAutoPay)
			changes.push({ field: "isAutoPay", before: bill.isAutoPay, after: value.isAutoPay });
