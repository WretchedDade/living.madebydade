...
export function BillPaidActivityAccordion({ activity }: { activity: BillPaidActivity }) {
    ...
    const paymentQuery = useQuery(convexQuery(api.billPayments.getById, { id: activity.targetId as Id<'billPayments'> }));
        ...
        paymentDetails = (
            <div className="mb-4 p-4 bg-zinc-900 border border-green-700 rounded-lg shadow-lg flex flex-col gap-4">
                <div className="flex items-center gap-3 mb-2">
                    <span className="inline-block px-3 py-1 rounded-full bg-green-600 text-white text-xs font-bold tracking-wide shadow">Paid {paymentQuery.data.datePaid ? new Date(paymentQuery.data.datePaid).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'}</span>
                </div>
                <div className="flex flex-col gap-1">
                    <div className="text-3xl font-extrabold text-green-300 mb-2">{formatCurrency(billQuery.data.amount)}</div>
                    <div className="flex gap-2">
                        <span className="font-semibold text-zinc-400">Due Date:</span>
                        <span className="text-zinc-200">{paymentQuery.data.dateDue ? new Date(paymentQuery.data.dateDue).toLocaleDateString(undefined, { dateStyle: 'medium' }) : 'N/A'}</span>
                    </div>
                    <div className="flex gap-2">
    ...
    return (
        ...
        >
            <div>
                ...
                {!paymentQuery.isSuccess || paymentQuery.data == null && (
                    <div className="text-zinc-400 italic">Payment details not found.</div>
                )}
                {!billQuery.isSuccess || billQuery.data == null && (
