...
export function RecentActivityAccordion({ activity, label, borderClass, children, staticRow }: RecentActivityAccordionProps) {
    if (staticRow) {
        return (
            <div className={`w-full flex items-center py-3 px-4 bg-transparent border-l-4 ${borderClass} mb-4`}> 
                ...
                <span className="text-zinc-500 mt-1 text-xs flex-shrink-0 min-w-0 break-words text-right ml-2">{formatRelativeTime(activity.timestamp)}</span>
            </div>
        );
    }
    return (
        <Accordion.Item value={activity._id} className='mb-4'>
            <Accordion.Header>
                <Accordion.Trigger className={`w-full flex items-center justify-between py-3 px-4 bg-transparent hover:bg-zinc-800 transition cursor-pointer border-l-4 ${borderClass}`}> 
                    ...
                    <span className="flex flex-row items-center ml-2 flex-shrink-0"> 
                        <span className="text-zinc-500 mt-1 sm:mt-0 text-xs flex-shrink-0 min-w-0 break-words text-right mr-2">{formatRelativeTime(activity.timestamp)}</span> 
